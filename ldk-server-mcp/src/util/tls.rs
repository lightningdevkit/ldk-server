...
// Mirrors `ldk-server/src/util/tls.rs`. Copied (rather than extracted into a shared
// crate) to keep the v1 PR scoped to a single new crate. See the brainstorm doc
// at docs/brainstorms/2026-05-07-ldk-server-mcp.md for the deferred refactor.
...
fn load_tls_config(cert_path: &str, key_path: &str) -> Result<ServerConfig, String> {
	...
	// Advertise both HTTP/2 and HTTP/1.1 so browsers (UI) and gRPC-style HTTP/2
	// clients (the future MCP transport) both negotiate cleanly.
...
mod tests {
	...
	fn test_generate_and_load_roundtrip() {
		...
		let cert_path = temp_dir.join(format!("test_mcp_tls_cert_{suffix}.pem"));
		let key_path = temp_dir.join(format!("test_mcp_tls_key_{suffix}.pem"));
		...
		assert!(res.is_ok(), "load_tls_config failed: {:?}", res.err());
	...
	#[test]
	fn test_generate_self_signed_cert_with_extra_hosts() {
		let temp_dir = std::env::temp_dir();
		let mut suffix_bytes = [0u8; 8];
		getrandom::getrandom(&mut suffix_bytes).unwrap();
		let suffix = u64::from_ne_bytes(suffix_bytes);
		let cert_path = temp_dir.join(format!("test_mcp_tls_san_cert_{suffix}.pem"));
		let key_path = temp_dir.join(format!("test_mcp_tls_san_key_{suffix}.pem"));
		...
		generate_self_signed_cert(
			...
			&["mcp.example.com".to_string(), "10.0.0.1".to_string()],
		)
		.unwrap();
