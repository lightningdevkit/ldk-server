...
use crate::mcp::{
	request_protocol_version, validate_client_metadata, DiscoverResult, ListToolsResult,
	PROTOCOL_VERSION,
};
use crate::protocol::{
...
async fn handle_request(
	...
	if !matches!(method.as_str(), "server/discover" | "tools/list" | "tools/call") {
		return error_response(id, METHOD_NOT_FOUND, format!("Method not found: {method}"));
	}

	let protocol_version = match request_protocol_version(params.as_ref()) {
	...
	if let Err(message) = validate_client_metadata(params.as_ref()) {
	...
	match method.as_str() {
		...
		_ => unreachable!("request method was validated above"),
	}
}
