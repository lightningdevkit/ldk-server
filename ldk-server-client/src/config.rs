...
///
/// Use `override_macaroon` if supplied. Otherwise, look for `admin.macaroon` in the configured
/// storage directory, then the default data directory.
/// Return an error if a file cannot be read, is too large, or contains an invalid token.
