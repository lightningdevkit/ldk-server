...
enum Commands {
	...
	Bolt11ClaimForHash {
		...
		payment_hash: String,
		#[arg(
			help = "The claimable amount, e.g. 50sat or 50000msat, this is only used for verifying we are claiming the expected amount. Put 0sat to skip."
		)]
...
async fn main() {
	...
	match cli.command {
		...
		Commands::Bolt11ClaimForHash { payment_hash, claimable_amount, preimage } => {
			handle_response_result::<_, Bolt11ClaimForHashResponse>(
				client
					.bolt11_claim_for_hash(Bolt11ClaimForHashRequest {
						payment_hash,
						claimable_amount_msat: claimable_amount.to_msat(),
						preimage,
					})
					.await,
			);
