...
///
/// Events are represented as [`Event`] messages defined in [`ldk_server_json_models::events`].
/// These events are serialized to JSON by the publisher before transmission, and consumers can
/// deserialize them using the type definitions in that module.
///
...
pub trait EventPublisher: Send + Sync {
	...
	///
	/// # Arguments
	/// * `event` - The event message to publish, provided as an [`Event`]
	///             defined in [`ldk_server_json_models::events`]. Implementors must serialize
	///             the [`Event`] to bytes before publishing.
	///
...
impl EventPublisher for NoopEventPublisher {
	...
	async fn publish(&self, _event: Event) -> Result<(), LdkServerError> {
		Ok(())
	}
}
