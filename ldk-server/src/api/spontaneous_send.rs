...
use ldk_server_json_models::api::{SpontaneousSendRequest, SpontaneousSendResponse};
...
pub(crate) fn handle_spontaneous_send_request(
	...
	let node_id = PublicKey::from_slice(&request.node_id).map_err(|_| {
	...
	let response = SpontaneousSendResponse { payment_id: payment_id.0 };
	Ok(response)
}
