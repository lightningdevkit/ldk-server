...
use ldk_server_json_models::api::{Bolt11ClaimForHashRequest, Bolt11ClaimForHashResponse};
...
pub(crate) fn handle_bolt11_claim_for_hash_request(
	...
	let preimage = PaymentPreimage(request.preimage);

	let payment_hash = if let Some(hash_bytes) = request.payment_hash {
