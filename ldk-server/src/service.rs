...
use crate::util::config::AuthConfig;
...
impl NodeService {
	pub(crate) fn new(
		node: Arc<Node>, paginated_kv_store: Arc<dyn PaginatedKVStore>, auth_config: AuthConfig,
	) -> Self {
...
fn extract_auth_params<B>(req: &Request<B>) -> Result<(u64, String), LdkServerError> {
	let auth_header = req
		.headers()
		.get("X-Auth")
	...
	Ok((timestamp, hmac_hex.to_string()))
}

/// Validates the HMAC authentication after the request body has been read.
fn validate_hmac_auth(
	timestamp: u64, provided_hmac_hex: &str, body: &[u8], auth_config: &AuthConfig,
	...
	let mut hmac_engine: HmacEngine<sha256::Hash> = HmacEngine::new(auth_config.api_key.as_bytes());
...
impl Service<Request<Incoming>> for NodeService {
	...
	fn call(&self, req: Request<Incoming>) -> Self::Future {
		...
		match &req.uri().path()[1..] {
			...
			OPEN_CHANNEL_PATH => Box::pin(handle_request(
				context,
				req,
				auth_params,
				auth_config,
...
>(
	context: Context, request: Request<Incoming>, auth_params: (u64, String),
	auth_config: AuthConfig, handler: F,
	...
	let (timestamp, provided_hmac) = auth_params;
	if let Err(e) = validate_hmac_auth(timestamp, &provided_hmac, &bytes, &auth_config) {
		let (error_response, status_code) = to_error_response(e);
		return Ok(Response::builder()
			.status(status_code)
			.body(Full::new(Bytes::from(error_response.encode_to_vec())))
...
mod tests {
	...
	fn test_validate_hmac_auth_success() {
		let auth_config = AuthConfig { api_key: "test_api_key".to_string() };
		let body = b"test request body";
		let timestamp =
			std::time::SystemTime::now().duration_since(std::time::UNIX_EPOCH).unwrap().as_secs();
		let hmac = compute_hmac(&auth_config.api_key, timestamp, body);

		let result = validate_hmac_auth(timestamp, &hmac, body, &auth_config);
		assert!(result.is_ok());
	}
	...
	fn test_validate_hmac_auth_tampered_body() {
		...
		let hmac = compute_hmac(&auth_config.api_key, timestamp, original_body);

		// Try to validate with tampered body
		let result = validate_hmac_auth(timestamp, &hmac, tampered_body, &auth_config);
		assert!(result.is_err());
		assert_eq!(result.unwrap_err().error_code, AuthError);
	}
}
