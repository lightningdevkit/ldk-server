...
fn main() {
			...
			match default {
				Some(path) => {
					...
					if config_file.network != Network::Bitcoin {
						path.join(config_file.network.to_string())
					} else {
						path
					}
				},
	...
	let log_file_path = config_file.log_file_path.map(PathBuf::from).unwrap_or_else(|| {
		let mut default_log_path = storage_dir.clone();
	...
	let api_key = match load_or_generate_api_key(&storage_dir) {
		Ok(key) => key,
		Err(e) => {
	...
	ldk_node_config.storage_dir_path = storage_dir.to_str().unwrap().to_string();
	...
	let paginated_store: Arc<dyn PaginatedKVStore> =
		Arc::new(match SqliteStore::new(storage_dir.clone(), None, None) {
			Ok(store) => store,
			Err(e) => {
