...
/// Bitcoin block hashes and txids are displayed in reversed byte order by convention.
/// `Hash::to_byte_array()` returns internal order; this helper reverses for display.
pub(crate) fn to_display_bytes(hash: [u8; 32]) -> [u8; 32] {
	let mut bytes = hash;
	bytes.reverse();
	bytes
}
...
pub(crate) fn channel_to_model(channel: ChannelDetails) -> Channel {
	Channel {
		...
		funding_txo: channel
			.funding_txo
			.map(|o| OutPoint { txid: to_display_bytes(o.txid.to_byte_array()), vout: o.vout }),
		...
		ConfirmationStatus::Confirmed { block_hash, height, timestamp } => {
			ldk_server_json_models::types::ConfirmationStatus::Confirmed(
				ldk_server_json_models::types::Confirmed {
					block_hash: to_display_bytes(block_hash.to_byte_array()),
					height,
					timestamp,
				},
			)
	...
	match pending_sweep_balance {
		...
		PendingSweepBalance::BroadcastAwaitingConfirmation {
			...
			ldk_server_json_models::types::BroadcastAwaitingConfirmation {
				channel_id: channel_id.map(|c| c.0),
				latest_broadcast_height,
				latest_spending_txid: to_display_bytes(latest_spending_txid.to_byte_array()),
				amount_satoshis,
			},
		),
		PendingSweepBalance::AwaitingThresholdConfirmations {
			...
			ldk_server_json_models::types::AwaitingThresholdConfirmations {
				channel_id: channel_id.map(|c| c.0),
				latest_spending_txid: to_display_bytes(latest_spending_txid.to_byte_array()),
				confirmation_hash: to_display_bytes(confirmation_hash.to_byte_array()),
				confirmation_height,
				amount_satoshis,
			},
		),
