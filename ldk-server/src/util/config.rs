...
impl ConfigBuilder {
	fn merge_toml(&mut self, toml: TomlConfig) {
		if let Some(node) = toml.node {
			...
			self.api_key = node.api_key.or(self.api_key.clone());
		}

		if let Some(storage) = toml.storage {
			self.storage_dir_path = storage.disk.dir_path.or(self.storage_dir_path.clone());
		}
	...
	fn merge_args(&mut self, args: &ArgsConfig) {
		...
		if let Some(node_listening_address) = &args.node_listening_address {
			self.listening_address = Some(node_listening_address.clone());
		}
	...
	fn build(self) -> io::Result<Config> {
		...
		let api_key = self.api_key.ok_or_else(|| missing_field_err("api_key"))?;
...
pub struct ArgsConfig {
	...
	#[arg(long, env = "LDK_SERVER_API_KEY")]
	api_key: Option<String>,
}
